...
class LocksmithScript < BaseDragonrealmsScript
  ...
  def validate_args(args)
    args.length >= 4 ||
    (args.length == 3 && config_lockpick_container) ||
    (args.length == 2 && config_lockpick && config_lockpick_container) ||
    (args.length == 1 && config_box_container && config_lockpick && config_lockpick_container)
  end

  def run(args)
    box = args[0]
    box_container = args[1] || config_box_container
    lockpick = args[2] || config_lockpick
    lockpick_container = args[3] || config_lockpick_container
    disarm_option = args[4] || config_disarm_option || 'careful'
    pick_option = args[5] || config_pick_option || 'careful'
    ...
    if \
      ...
      if identify_and_disarm(box, disarm_option)
        ...
        identify_and_pick(box, pick_option)
      end
    end
