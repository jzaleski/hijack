require 'spec_helper'
require 'helpers/config_helpers'

describe ConfigHelpers do

  class InvalidDummyInterface

    include ConfigHelpers

  end

  class ValidDummyInterface

    include ConfigHelpers

    def config
      @config ||= {}
    end

  end

  describe 'config' do

    before do
      @interface = InvalidDummyInterface.new
    end

    it 'will raise an error if config is not overridden' do
      # to exercise this behavior any method can be called here
      expect{@interface.process_args(['--test=true'])}.to \
        raise_exception(%{All "#{ConfigHelpers}(s)" must override the "config" method})
    end

  end

  describe 'process_args' do

    before do
      @interface = ValidDummyInterface.new
    end

    it 'will update config with parsed values' do
      @interface.process_args(['--test=true', '--hello=world'])
      @interface.config.should == {:test => 'true', :hello => 'world'}
    end

    it 'will ignore args that are not properly formatted' do
      @interface.process_args(['--test=true', 'hello=world'])
      @interface.config.should == {:test => 'true'}
    end

  end

  describe 'process_config_file' do

    class DummyFile

      def initialize(lines)
        @lines = lines
      end

      def each_line
        @lines.each do |line|
          yield line
        end
      end

    end

    before do
      @interface = ValidDummyInterface.new
    end

    it 'will short-circuit if the config_file does not exist' do
      File.should_receive(:exist?).with('config_file.conf')
      File.should_not_receive(:new)
      @interface.process_config_file('config_file.conf')
    end

    it 'will process a config_file if it exists' do
      File.stub(:exist?).and_return(true)
      File.stub(:new).and_return(DummyFile.new(['test=true', 'hello=world']))
      @interface.process_config_file('config_file.conf')
      @interface.config.should == {:test => 'true', :hello => 'world'}
    end

    it 'will ignore comments' do
      File.stub(:exist?).and_return(true)
      File.stub(:new).and_return(DummyFile.new(['# comment', 'test=true']))
      @interface.process_config_file('config_file.conf')
      @interface.config.should == {:test => 'true'}
    end

    it 'will parse JSON encoded arrays' do
      File.stub(:exist?).and_return(true)
      File.stub(:new).and_return(DummyFile.new(['test=["hello", "world"]']))
      @interface.process_config_file('config_file.conf')
      @interface.config.should == {:test => ['hello', 'world']}
    end

    it 'will parse JSON encoded hashes' do
      File.stub(:exist?).and_return(true)
      File.stub(:new).and_return(DummyFile.new(['test={"hello": "world"}']))
      @interface.process_config_file('config_file.conf')
      @interface.config.should == {:test => {:hello => 'world'}}
    end

  end

end
